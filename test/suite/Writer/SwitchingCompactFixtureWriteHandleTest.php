 ...
 * @covers \Eloquent\Fixie\Writer\SwitchingCompactFixtureWriteHandle
 * @covers \Eloquent\Fixie\Handle\AbstractHandle
 */
class SwitchingCompactFixtureWriteHandleTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testConstructor()
    {
        ...
        $handle = new SwitchingCompactFixtureWriteHandle($stream, 'foo', $this->renderer, $this->isolator);

        $this->assertSame($stream, $handle->stream());
        $this->assertSame('foo', $handle->path());
        $this->assertSame($this->renderer, $handle->renderer());
        $this->assertSame(10485760, $handle->bufferSize());
    }

    public function testConstructorDefaults()
    {
        $this->streams[] = $stream = fopen('data://text/plain;base64,', 'wb');
        $handle = new SwitchingCompactFixtureWriteHandle($stream);
    ...
    {
        $handle = new SwitchingCompactFixtureWriteHandle($this->streamFixture());
        $handle->setBufferSize(111);

        $this->assertSame(111, $handle->bufferSize());
    }

    public function testStreamFailureClosed()
    {
        $handle = new SwitchingCompactFixtureWriteHandle(null, 'foo', null, $this->isolator);
        $handle->close();
    ...
    {
        $stream = $this->streamFixture();
        $handle = new SwitchingCompactFixtureWriteHandle($stream, null, null, $this->isolator);
        $handle->close();
    ...
    public function testCloseFailureFcloseError()
    {
        ...
        $handle = new SwitchingCompactFixtureWriteHandle($this->streamFixture(), null, null, $this->isolator);
...
EOD;
        $data['Exceeding buffer size'] = array($expected, $rows, 1);
     ...
     */
    public function testWriter($expected, array $rows, $bufferSize = 10485760)
    {
        $handle = new SwitchingCompactFixtureWriteHandle($this->streamFixture(), null, null, $this->isolator);
        $handle->setBufferSize($bufferSize);
        foreach ($rows as $row) {
            $handle->write($row);
        }
        $handle->close();
     ...
     * @dataProvider writerData
     */
    public function testWriterWriteAll($expected, array $rows, $bufferSize = 10485760)
    {
        $handle = new SwitchingCompactFixtureWriteHandle($this->streamFixture(), null, null, $this->isolator);
        $handle->setBufferSize($bufferSize);
